...
import { buildMods, pathInRequest, effectiveRisk } from './lib/buildMods.mjs';
import YAML from 'yaml';
import { JSONRPCClient, JSONRPCServer } from 'json-rpc-2.0';
import { SecurityScanner, KNOWN_COMMANDS, checkNoPrototypePollution, checkDataForLiterals } from './security_scanner.mjs';
...
const commands = {
    ...
    IMPORT: async ([src], ctx) => {
        ...
        const content = url.startsWith('http')
            ...
            : YAML.parse(fs.readFileSync(url, 'utf8'));
        ...
        if (Array.isArray(content)) {
            ...
            if (content.length > 0 && Array.isArray(content[0]) && content[0][0] === 'REQUEST') {
                ...
                if (ctx.scanner) {
                    for (const req of libRequestList) {
                        ...
                        ctx.scanner.checkPath(req, true);
                        if (ctx.scanner.requestList !== null &&
                            !pathInRequest(req, ctx.scanner.requestList)) {
                            throw new Error(
            ...
            const libScanner = ctx.scanner
                ...
                : null;
            if (libScanner && libRequestList !== null) libScanner.requestList = libRequestList;
    ...
    WORKER: async ([id, code], ctx) => {
        const spawnId = resolveArgs(id, ctx);
        const spawnCode = resolveArgs(code, ctx);
        const childPolicy = ctx.scanner ? ctx.scanner.manifest : { maxRisk: 'LOW' };
        const childCapabilities = ctx.scanner ? ctx.scanner.requestList : null;
        ctx.vars["$LAST"] = await ctx.rpcClient.request('Worker.spawn', { id: spawnId, code: spawnCode, policy: childPolicy, parentCapabilities: childCapabilities });
    },
    RPC_EXPOSE: ([methodName, fnName], ctx) => {
        const name = resolveArgs(methodName, ctx);
        const localFn = resolveArgs(fnName, ctx);
        if (!ctx.functions[localFn]) throw new Error(`Fn Undefined: ${localFn}`);
        ctx.rpcServer.addMethod(name, async (params) => {
            // Create an isolated sub-context using SANDBOX configuration
            const isolatedCtx = { ...ctx, vars: structuredClone(ctx.vars) };
            await commands.CALL([localFn, ["LITERAL", params]], isolatedCtx, false);
            return isolatedCtx.vars["$LAST"];
        });
    },
    RPC_CALL: async ([targetId, method, params], ctx) => {
        ctx.vars["$LAST"] = await ctx.rpcClient.request('Worker.rpc_call', { targetId: resolveArgs(targetId, ctx), method: resolveArgs(method, ctx), params: resolveArgs(params, ctx) });
    },
    CPU: async (args, ctx, em) => {
        if (ctx.mon.quotaArray) Atomics.add(ctx.mon.quotaArray, 2, 1);
        try {
            // Evaluate the sub-steps in args as inline execution blocks
            await commands.EXEC(args, ctx, em);
        } finally {
            if (ctx.mon.quotaArray) Atomics.sub(ctx.mon.quotaArray, 2, 1);
        }
    }
};
...
parentPort.on('message', (msg) => {
    if (msg.method) rpcServer.receive(msg).then((response) => { if (response) parentPort.postMessage(response); });
    else rpcClient.receive(msg);
});

async function initWorker() {
    const { scannedProg, policy, registryEntries } = workerData;

    const scanner = new SecurityScanner(policy, registryEntries);
    const mods = await buildMods(registryEntries, policy.maxRisk ?? 'LOW', workerData.requestList);
    const quotaArray = workerData.quotaSab ? new Int32Array(workerData.quotaSab) : null;
    const mon = {
        quotaArray,
        usage: { fsBytes: 0, cpuMs: 0, memoryMb: 0 },
        checkResources: () => {
            if (quotaArray && Atomics.load(quotaArray, 0) === 1) throw new Error("QUOTA_EXCEEDED");
        },
        instrumentFs: (fsModule) => {
            const originalWrite = fsModule.writeFileSync;
            fsModule.writeFileSync = (path, data, ...args) => {
                const bytes = Buffer.byteLength(data);
                if (quotaArray) Atomics.add(quotaArray, 1, bytes);
    ...
    const ctx = {
        mods,
        vars: { "$LAST": null, "$ERROR": null, "$ITEM": null, "$USAGE": null, "$INPUT": null, "$RETURN": null, "$PROGRAM": workerData.program, "$WORKERID": workerData.workerId },
        functions: {},
        mon,
        scanner,
        rpcClient,
    ...
    if (workerData.requestList) ctx.scanner.requestList = workerData.requestList;
