...
| nejy run tests/programs/use-promise-all.yaml --policy LOW (expected ok: true) | nejy run tests/programs/use-promise-all.yaml --policy LOW (expected ok: true) | OVERSTATED | The integration test checks exit code and yaml result, but ignores `expectReturn` which is declared in `tests/programs/use-promise-all.yaml`. Thus the description claims it expects a certain return value, but the test doesn't check it. |
