...
export const commands = {
    ...
    DEF: ([name, steps], ctx) => { /* To be overwritten in Commit 4 */ },
    REQUEST: () => {
        ...
        if (Array.isArray(content)) {
            ...
            for (const step of libBody) {
                ...
                if (cmd === 'DEF') {
                    ...
                    if (name && body) {} // To be overwritten in Commit 4
                }
    ...
    for (const step of steps) {
        try {
            ...
            if (err.message === "QUOTA_EXCEEDED" && !em) {
                ...
                if (ctx.mods.ON_QUOTA) await ctx.mods.ON_QUOTA();
                throw err;
            }
            throw err;
        }
