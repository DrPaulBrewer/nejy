...
import fsProxy from '../fsProxy.mjs';
import YAML from 'yaml';

const HANDLED_COMMANDS = new Set([
    "EXEC", "NEW", "PIPE", "IMPORT", "LITERAL", "IF", "FOR_EACH", "TRY", "DEF", "TO", "SANDBOX", "SET", "REQUEST"
]);
...
export class SecurityScanner {
    ...
    async scan(program) {
        ...
        await this.hoistAndAnalyzeImports(steps);
        await this.analyze(steps);
        return steps;
    }

    async hoistAndAnalyzeImports(steps) {
        ...
        for (const step of steps) {
            ...
            if (path === 'IMPORT') {
                ...
                if (typeof src !== 'string' || src.startsWith('$')) {
                     throw new Error(`SEC_BLOCK: IMPORT argument must be a static literal string during scan`);
                }
                importPromises.push(this.loadAndScanImport(src));
            }
        }
        await Promise.all(importPromises);
    }

    async loadAndScanImport(src) {
        const content = src.startsWith('http')
            ? await (await fetch(src)).json()
            : YAML.parse(fsProxy.readFileSync(src, 'utf8'));
        ...
        if (Array.isArray(content)) {
            ...
            if (content.length > 0 && Array.isArray(content[0]) && content[0][0] === 'REQUEST') {
                ...
                for (const req of libRequestList) {
                    ...
                    this.checkPath(req, true);
                    if (this.requestList !== null && !pathInRequest(req, this.requestList)) {
                        throw new Error(
            ...
            const libScanner = new SecurityScanner(this.manifest, this.registryEntries);
            libScanner.requestList = libRequestList;

            for (const step of libBody) {
                if (!Array.isArray(step)) continue;
                const [cmd] = step;
                if (cmd === 'IMPORT') {
                     throw new Error(`SEC_BLOCK: IMPORT commands are not allowed inside imported libraries`);
                }
            }
            await libScanner.analyze(libBody);
        } else if (content && typeof content === 'object') {
            for (const [name, body] of Object.entries(content)) {
                await this.analyze(body);
            }
        }
    }
    ...
    async analyze(steps, inDef = false) {
        if (!Array.isArray(steps)) return;
        for (const step of steps) {
            if (!Array.isArray(step)) continue;
            const [path, args = []] = step;
            ...
            if (path === "IMPORT") {
                if (inDef) {
                    throw new Error(`SEC_BLOCK: IMPORT commands are not allowed inside DEF bodies`);
                }
                if (this.riskMap["LOW"] > this.currentRisk)
                    throw new Error(`SEC_BLOCK: 'IMPORT' requires LOW risk (Manifest: ${this.manifest.maxRisk})`);
            }
            ...
            if (path === 'IF') {
                ...
                if (Array.isArray(args[0])) await this.analyze([args[0]], inDef);  // cond as inline step
                if (Array.isArray(args[1])) await this.analyze(args[1], inDef);    // true branch
                if (Array.isArray(args[2])) await this.analyze(args[2], inDef);    // false branch
            } else if (path === 'FOR_EACH') {
                if (Array.isArray(args[1])) await this.analyze(args[1], inDef);    // loop body
            } else if (path === 'TRY') {
                if (Array.isArray(args[0])) await this.analyze(args[0], inDef);    // try block
                if (Array.isArray(args[1])) await this.analyze(args[1], inDef);    // catch block
            } else if (path === 'DEF') {
                // Remember that in the new syntax, DEF args are [name, params, body]
                // but currently it's still treated as [name, steps] until Commit 4
                // We'll update the indexing to [2] in Commit 4 when we update the structure.
                // For now, it's just args[1].
                if (Array.isArray(args[1])) await this.analyze(args[1], true);    // function body
            } else if (path === 'TO') {
                // Same disambiguation as the interpreter: string-first = single step, else block
                const code = args[1];
                if (Array.isArray(code)) {
                    if (typeof code[0] === 'string') await this.analyze([code], inDef);  // single step
                    else await this.analyze(code, inDef);                                // block of steps
                }
                ...
                await childScanner.hoistAndAnalyzeImports(subprogram);
                await childScanner.analyze(subprogram, inDef);
            }
            // PIPE: already fully handled in the explicit PIPE block above.
            // EXEC, NEW, SET, IMPORT: args are data — no branch recursion.
        }
    }
}
