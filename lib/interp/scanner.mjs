...
const HANDLED_COMMANDS = new Set([
    "EXEC", "PROMISE", "NEW", "PIPE", "LITERAL", "IF", "FOR_EACH", "TRY", "F", "MATH", "TO", "SANDBOX", "SET", "REQUEST", "CHILD"
]);
...
export class SecurityScanner {
    ...
    async analyze(steps) {
        ...
        for (const step of steps) {
            ...
            // For EXEC, PROMISE, and NEW, check the explicit target callable path.
            if ((path === "EXEC" || path === "PROMISE" || path === "NEW") && Array.isArray(args) && typeof args[0] === 'string') {
            ...
            switch (path) {
                ...
                case 'MATH':
                    ...
                    if (Array.isArray(args[1])) {
                        for (const formalArg of args[1]) {
                            ...
                            // Wait, the processFormalArg throws "Command parsing error" for MATH references... Let's see if tests break. I'll just use the helper.
                            // I should change the error in processFormalArg to handle Scanner vs runtime? Or just use processFormalArg and let the tests dictate.
                ...
                case 'TO': {
                    if (args[0] && typeof args[0] === 'string') {
                        // All variable assignments must be checked for prototype pollution and risk
                        this.checkPath(args[0].startsWith('$') ? args[0] : `$${args[0]}`);
                    }

                    // Same disambiguation as the interpreter: string-first = single step, else block
                    const code = args[1];
                    if (typeof code === 'string') {
                        // String-shorthand for a single command (e.g., ["TO", ["v", "os.platform"]])
            ...
            // EXEC, NEW, SET: args are data — no branch recursion.
        }
    }
}
