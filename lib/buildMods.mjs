...
import Ajv from 'ajv';
import { RegistryFileSchema } from './schema/registry.mjs';
...
const validateRegistryFile = ajv.compile(RegistryFileSchema);
 ...
 */
export function pathInRequest(pathStr, requestList) {
...
async function buildEntry(entry, maxLevel, requestList) {
  const {
    key,
    src,
    module: modulePath,
  ...
  // Anti-prototype-pollution: freeze module prototypes if not explicitly disabled
  if (entry.freezePrototypes !== false) {
    const freezeAndVerify = (protoObj, errMsgPrefix) => {
      if (!protoObj) return;
      Object.freeze(protoObj);
      const descs = Object.getOwnPropertyDescriptors(protoObj);
      for (const key of Reflect.ownKeys(protoObj)) {
        const desc = descs[key];
        if (desc && (desc.configurable || desc.writable)) {
          throw new Error(`FATAL SECURITY ERROR: ${errMsgPrefix}${String(key)} is not frozen!`);
        }
      }
    };
  ...
  if (methods && methods !== '*') {
    ...
    for (const [methodPath, methodRisk] of Object.entries(methods)) {
      ...
      if (requestList && !pathInRequest(methodPath, requestList)) continue;
      ...
      const val = getProperty(instance, methodName);
      ...
      if (typeof val === 'function') {
        ...
        const parentPath = methodName.includes('.')
          ...
          : null;
        const parentObj = parentPath ? getProperty(instance, parentPath) : instance;
    ...
    const proxy = new Proxy(instance, {
      get(target, prop, receiver) {
        ...
        if (requestList && !pathInRequest(fullPath, requestList)) return undefined;
      ...
      has(target, prop) {
        ...
        const fullPath = `${key}.${prop}`;
        if (requestList && !pathInRequest(fullPath, requestList)) return false;
        const effectiveRisk = overrides[fullPath] ?? baseRisk;
...
/**
 * Parse registry files into a flat array of entry objects.
 * Used by SecurityScanner so it can share parsed data with buildMods()
 * without double-loading.
 *
 * @param {string[]} registryFiles - paths to YAML registry files
 * @returns {object[]}             - flat array of all entry objects
 */
export function loadRegistry(registryFiles) {
  const allEntries = [];
  for (const filePath of registryFiles) {
    try {
      const rawDoc = YAML.parse(readFileSync(filePath, 'utf8'));
      if (!rawDoc || typeof rawDoc !== 'object') continue;
      if (!rawDoc.entries) continue; // Skip empty docs nicely

      const isValid = validateRegistryFile(rawDoc);
      if (!isValid) {
          const errors = validateRegistryFile.errors.map(err => {
              const path = err.instancePath.replace(/^\//, '').replace(/\//g, '.');
              return `  - ${path || 'root'}: ${err.message}`;
          }).join('\n');
          throw new Error(`Registry Validation Error in ${filePath}:\n${errors}`);
      }

      if (rawDoc && rawDoc.entries) {
          allEntries.push(...rawDoc.entries);
      }
    } catch (e) {
      process.stderr.write(`loadRegistry: error loading '${filePath}': ${e.message}\n`);
    }
  }
  return allEntries;
}
 ...
 */
export async function buildMods(registryEntries, maxRisk = 'LOW', requestList = null) {
  const maxLevel = RISK_MAP[maxRisk] ?? 0;
  const mods = Object.create(null); // no prototype — clean slate

  if (!Array.isArray(registryEntries)) return mods;

  for (const entry of registryEntries) {
    try {
      const [key, value] = await buildEntry(entry, maxLevel, requestList);
...
export function effectiveRisk(pathStr, registry) {
  ...
  for (const entry of registry) {
    ...
    // If we request a module root and it's in EXPLICIT mode, 
    // its risk is the minimum risk of any of its methods.
    if (pathStr === entry.key && methods) {
       let minLevel = 4; // INSANE + 1
       let bestRisk = null;
       for (const r of Object.values(methods)) {
           const l = RISK_MAP[r] ?? 3;
           if (l < minLevel) { minLevel = l; bestRisk = r; }
       }
       if (bestRisk) return bestRisk;
    }
  }
