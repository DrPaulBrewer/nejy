...
async function buildEntry(entry, maxLevel, requestList) {
  ...
  if (entry.freezePrototypes !== false) {
    if (rawModule && rawModule.prototype) {
      Object.freeze(rawModule.prototype);
      const _descs = Object.getOwnPropertyDescriptors(rawModule.prototype);
      for (const key of Reflect.ownKeys(rawModule.prototype)) {
        const desc = _descs[key];
        if (desc && (desc.configurable || desc.writable)) {
          throw new Error(`FATAL SECURITY ERROR: rawModule.prototype.${key} is not frozen!`);
        }
      }
    }
    if (instance && instance.prototype && instance !== rawModule) {
      Object.freeze(instance.prototype);
      const _descs = Object.getOwnPropertyDescriptors(instance.prototype);
      for (const key of Reflect.ownKeys(instance.prototype)) {
        const desc = _descs[key];
        if (desc && (desc.configurable || desc.writable)) {
          throw new Error(`FATAL SECURITY ERROR: instance.prototype.${key} is not frozen!`);
        }
      }
    }
      ...
      for (const key of Object.getOwnPropertyNames(instance)) {
        try {
          ...
          if (val && typeof val === 'function' && val.prototype) {
            ...
            const _descs = Object.getOwnPropertyDescriptors(val.prototype);
            for (const k of Reflect.ownKeys(val.prototype)) {
              const desc = _descs[k];
              if (desc && (desc.configurable || desc.writable)) {
                 throw new Error(`FATAL SECURITY ERROR: method prototype ${k} is not frozen!`);
              }
            }
          }
        } catch(e) {
...
export function loadRegistry(registryFiles) {
  ...
  for (const filePath of registryFiles) {
    try {
      ...
      if (Array.isArray(doc?.entries)) allEntries.push(...doc.entries);
    } catch (e) {
