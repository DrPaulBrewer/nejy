...
import { run } from './interp/commands.mjs';
import { buildMods } from './buildMods.mjs';
import ResourceMonitor from '../monitor/index.js';

const HANDLED_COMMANDS = [
    "EXEC", "NEW", "LITERAL", "IF", "FOR_EACH", "TRY", "F", "MATH", "SANDBOX", "SET", "REQUEST", "CHILD", "AWAIT", "PROMISE", "PIPE", "RETURN"
];
...
export async function startREPL(format, options) {
    ...
    const rl = readline.createInterface({
        ...
        completer: (line) => {
            ...
            let completions = [...HANDLED_COMMANDS, ".help", ".exit", ".quit", ".reset"];
            ...
            if (nejyState && nejyState.ctx) {
                ...
                if (currentWord.includes('.')) {
                    ...
                    if (parts.length === 2 && nejyState.ctx.mods[parts[0]]) {
                        const modName = parts[0];
                        const modPrefix = modName + '.';
                        const modProps = Object.keys(nejyState.ctx.mods[modName]);
                        completions.push(...modProps.map(p => modPrefix + p));
                    }
                } else {
    ...
    rl.on('line', async (line) => {
        ...
        if (isComplete && buffer.trim() !== '') {
            ...
            try {
                ...
                // Wrap in array if single step object or not an array
                if (!Array.isArray(parsedSteps) || (parsedSteps.length > 0 && !Array.isArray(parsedSteps[0]) && typeof parsedSteps[0] === 'string' && HANDLED_COMMANDS.includes(parsedSteps[0]))) {
