...
import { getHintForLine } from './hints.mjs';
import { runWizard, formatStep } from './repl-wizard.mjs';
...
export function getCompleter(format, nejyState, getBuffer = () => '') {
    return (line) => {
        const buffer = getBuffer();
        const isBufferEmpty = !buffer || buffer.trim() === '';
        const isLineEmpty = !line || line.trim() === '';
        const dotCommands = [".help", ".exit", ".quit", ".reset", ".vars", ".time", ".monitor", ".wiz"];
...
export async function startREPL(format, options) {
    ...
    let wizardActive = false; // reentrancy guard — prevents concurrent .wiz invocations

    const rl = readline.createInterface({
        input: process.stdin,
        output: process.stdout,
        completer: getCompleter(format, nejyState, () => buffer)
    });

    const mainPrompt = theme.prompt('nejy> ');
    const contPrompt = theme.prompt('... ');

    // Hook to display hints above the prompt as the user types
    let lastHint = '';
    let lastHintLines = 1; // Default is 1 blank line
    const originalRefreshLine = rl._refreshLine.bind(rl);
    rl._refreshLine = function() {
        // We only show hints on the main prompt (when buffer is empty)
        let hint = '';
        if (buffer.trim() === '') {
            hint = getHintForLine(this.line, format);

            if (hint !== lastHint) {
                lastHint = hint;
                // Clear current line first before moving up
                process.stdout.write('\x1b[2K\r');
                // Move up and clear the lines from the previous hint
                for (let i = 0; i < lastHintLines; i++) {
                    process.stdout.write('\x1b[1A\x1b[2K\r');
                }

                if (hint) {
                    // Print the new hint(s)
                    const lines = hint.split('\n');
                    lastHintLines = lines.length;
                    for (const line of lines) {
                        process.stdout.write(theme.dim(line) + '\n');
                    }
                } else {
                    // Keep the blank line structure
                    lastHintLines = 1;
    ...
    console.log(''); // Leave initial room for the hint
    ...
    rl.on('line', async (line) => {
        ...
        // .wiz is context-sensitive: works from both empty and non-empty buffer
        if (trimmed === '.wiz') {
            // Guard against concurrent invocations: a buffered \n from clack's last prompt
            // can reach readline immediately on stdin.resume() and fire a second .wiz handler.
            if (wizardActive) {
                rl.prompt();
                return;
            }
            wizardActive = true;
            const contextBuffer = buffer; // snapshot before clearing
            buffer = '';
            rl.setPrompt(mainPrompt);

            // Do NOT pause readline: clack owns stdin via raw mode while prompts are active,
            // so readline cannot fire 'line' events during that time anyway.
            // Pausing/resuming readline around clack corrupts its internal state and causes
            // the REPL to exit on the first blank line after the wizard completes.
            try {
                const result = await runWizard(format, contextBuffer);
                if (result && result.step) {
                    if (result.execute) {
                        // Run through the normal scan → build → run pipeline
                        try {
                            const step = result.step;
                            const scannedProg = (await nejyState.scanner.scan([step])) ?? [step];
                            ...
                            nejyState.ctx.vars['$RETURN'] = undefined;
                            await run(scannedProg, nejyState.ctx, false);
                            const ret = nejyState.ctx.vars['$RETURN'];
                            if (ret !== undefined && ret !== null) {
                                console.log(theme.var(`$RETURN: `) + theme.success(
                                    typeof ret === 'object'
                                        ? (format === 'json' ? JSON.stringify(ret, null, 2) : YAML.stringify(ret).trim())
                                        : String(ret)
                                ));
                            } else {
                        ...
                        // Print only
                        console.log(theme.dim('Step (not executed):'));
                        console.log(theme.var(formatStep(result.step, format)));
                    }
                }
            } catch (e) {
                console.error(theme.error(`Wizard Error: ${e.message}`));
            } finally {
                wizardActive = false;
                // Clack calls process.stdin.pause() when each prompt closes — resume stdin
                // so readline can accept keyboard input again.
                // Clack also calls process.stdin.unref() so the process can exit cleanly in
                // pure CLI tools; re-ref it here so our REPL process stays alive.
                process.stdin.resume();
                process.stdin.ref();
                console.log(''); // room for the hint line
                rl.prompt();
            }
            return;
        }
        ...
        if (buffer === '') {
            ...
            if (trimmed === '.help') {
                ...
                console.log(theme.cmd(`  .monitor      `) + `- Toggle resource monitor (CPU/memory/FS limits) ON/OFF`);
                console.log(theme.cmd(`  .wiz          `) + `- Launch step-builder wizard (context-sensitive: fills subprogram slots when buffer has a step)`);
            ...
            if (trimmed === '.monitor') {
                ...
                if (monitorEnabled) {
                    nejyState.mon.enable();
                } else {
                    nejyState.mon.disable();
                }
                console.log(theme.dim(`.monitor is now ${monitorEnabled ? 'ON' : 'OFF'}`));
                rl.prompt();
                return;
            }
            if (trimmed === '.reset') {
                try {
                    ...
                    let mon = new ResourceMonitor(policy.quotas);
                    if (mods.fs) mon.instrumentFs(mods.fs);
                    // Preserve the current monitor enabled/disabled state across resets
                    if (!monitorEnabled) mon.disable();
        ...
        if (isComplete && buffer.trim() !== '') {
            ...
            try {
                ...
                // Explicitly check for malformed strict-array commands before scanning/execution
                const strictArrayCommands = new Set(["F", "MATH", "EXEC", "TRY", "FOR_EACH", "SANDBOX", "IF", "AWAIT", "NEW"]);
                for (const step of parsedSteps) {
                    if (Array.isArray(step) && step.length > 0) {
                        const path = step[0];
                        if (strictArrayCommands.has(path) && step.length >= 2) {
                            if (!Array.isArray(step[1])) {
                                throw new Error(`'${path}' command expects its arguments to be grouped in an array, like ["${path}", ["arg1", "arg2"], ...]`);
                            }
                        }
                    }
                }
                ...
                if (showVars) {
                    ...
                    for (const [k, v] of Object.entries(nejyState.ctx.vars)) {
                        ...
                        if (typeof v === 'function') {
                            const h = nejyState.ctx.history.find(entry => Array.isArray(entry[1]) && entry[1][0] === k.replace('$', ''));
                            if (h && h[0] === 'F') {
                                safeVars[k] = "F...";
                            } else if (h && h[0] === 'MATH') {
                                safeVars[k] = "MATH...";
                        ...
                        if (repr && repr.length > 200) {
                            ...
                            // If we already JSON/YAML stringified it, we don't want the outer
                            // JSON.stringify(safeVars) to escape the quotes.
                            // However, we just want to parse it back so the final outer stringify is clean.
                            try {
                                safeVars[k] = (format === 'json' && repr !== String(v)) ? JSON.parse(repr) : v;
                            } catch (e) {
                                safeVars[k] = v;
                            }
                        }
